...
export default function GameBoard({onSelectSquare,board}){//GameBoard bileşenini initialgameboard üzerinde döngü yaparak oluşturuyoruz
...
*/
  ...
  return <ol id="game-board">
      {board.map((row,rowIndex)=>(// row her satırı temsil ediyor,rowIndex o satırın indexini
          <li key={rowIndex}>
          <ol>
              {row.map((playerSymbol,colIndex)=>(
                  <li key={colIndex}>
                      <button onClick={()=>onSelectSquare(rowIndex,colIndex)} disabled={playerSymbol!==null}>{playerSymbol}</button>
                      </li>//playerSymbol null'e eşit değilse true olur ve disabled yani buton devre dışı kalır
                      ))}
                      </ol>
      </li>))}
  </ol>
